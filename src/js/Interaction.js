...
export class Interaction {
  ...
  scoring(type, counts) {
    //type:1,2每次放下一个+20，每次消除一行每个格子+2
  ...
  _isOver() {
    ...
    for (let point of cells) {
      ...
      //循环-判断每一个点
      point: for (let index = 0, len = targetPoi.length; index < len; index++) {
  ...
  _getLine(point, dirction, ids) {
    ...
    let resArr = Array.prototype.concat.call(
      [],
      p3Arr.reverse(),
      p0Arr.splice(1)
    );
  ...
  _canDrop() {
    ...
    let inBound = !(
      bound.min.x + this.stage.r < sbound.min.x ||
  ...
  bindEvent(target, stage) {
    ...
    can.addEventListener(event.start, e => {
      ...
      if (this.target.isPointIn(mousePoint.x, mousePoint.y)) {
