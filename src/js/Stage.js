...
export class Stage {
   ...
   * @param {*} border
   */
  constructor(ctx, width, height, r = 10, fill = '#fff', border = '#eee') {
    this.ctx = ctx;
    this.width = width;
    this.height = height;
    this.r = r;
    this.fill = fill;
    this.border = border;
    this.mistake = 0.1; //精度误差范围（用于判断两点为同一点）
    this.cp = {}; //舞台中间点
    this.lastDp = {};
    this.dp = {}; //正被探测的点 mark-标记每条边对应点探测情况 done-本点被探测情况
    this.dCells = []; //需要被探测的点
    this.cells = []; //所有的六边形点
    this.boundRect = {};
  }
    ...
    while (this.dCells.length > 0) {
      ...
      if (!this.dp.done) {
        this.dp.mark.forEach((item, index) => {
          ...
          let dirctions = this._mapSource(index, [
            false,
            false,
            false,
            false,
  ...
  _verifyPoint(p) {
    ...
    for (let i = 0, len = dCells.length; i < len; i++) {
      if (
        Math.abs(dCells[i].x - p.x) <= this.mistake &&
        Math.abs(dCells[i].y - p.y) <= this.mistake
      ) {
        isPushed = true;
        order = i;
        break;
    ...
    let isOut =
      p.x - this.r < 0 ||
      p.x + this.r > this.width ||
      p.y - this.r < 0 ||
      p.y + this.r > this.height;
    return {
      isPushed: isPushed,
      isOut: isOut,
      order: order,
