...
export class Irregular {
   ...
   * @param {*} border
   */
  constructor(ctx, x, y, r = 10, maxLength = 3, border = '#fff') {
    this.ctx = ctx;
    this.cp = {
      x,
      y,
  ...
  _verifyPoint(p) {
    ...
    for (let item of dCells) {
      if (
        Math.abs(item.x - p.x) < this.mistake &&
        Math.abs(item.y - p.y) < this.mistake
      ) {
        isPushed = true;
        break;
      }
   ...
   */
  isPointIn(x, y) {
    const { min, max } = this.boundRect;
    return x - min.x > 0 && x - max.x < 0 && y - min.y > 0 && y - max.y < 0;
  }
  move(x, y) {
